...
class ViewController: UIViewController {
        ...
        if loginEmail.text != "" && loginPassword.text != "" {
            ...
            if isValidEmail(loginEmail.text!) == true{
                ...
                if loginPassword.text!.count > 8 {
                    ...
                    Auth.auth().signIn(withEmail: loginEmail.text!, password: loginPassword.text!) { authdata, error in
                        if let error = error {
    ...
    @IBAction func loginSubscribe(_ sender: Any) {
        ...
        Auth.auth().createUser(withEmail: loginEmail.text!, password: loginPassword.text!) { authdata, error in
            if let error = error {
